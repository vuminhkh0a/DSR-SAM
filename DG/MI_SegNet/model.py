...
class SegEncoder(nn.Module):
    def __init__(self, in_channels=3, init_features=64, num_blocks=2):
        super().__init__()
...
class SegDecoder(nn.Module):
    def __init__(self, out_channels=1, init_features=64, num_blocks=2):
        super().__init__()
        features = init_features
...
class ReconEncoder(nn.Module):
    def __init__(self, in_channels=3, init_features=16):
        super().__init__()
        features = init_features
        self.encoder = nn.Sequential(
            nn.Conv2d(in_channels, features, kernel_size=3, padding=1),
            nn.GroupNorm(num_groups=max(1, features // 16), num_channels=features),
            nn.ReLU(inplace=True),
            ...
            nn.GroupNorm(num_groups=max(1, features // 8), num_channels=2 * features),
            nn.ReLU(inplace=True),
            ...
            nn.GroupNorm(num_groups=max(1, features // 4), num_channels=4 * features),
            nn.ReLU(inplace=True),
            ...
            nn.GroupNorm(num_groups=max(1, features // 2), num_channels=8 * features),
            nn.ReLU(inplace=True),
...
class ReconDecoder(nn.Module):
    def __init__(self, in_channels_a, in_channels_d, out_channels=3, init_features=16):
        super().__init__()
        features = init_features
        self.decoder = nn.Sequential(
    ...
    def forward(self, z_a, z_d):
        return self.decoder(torch.cat([z_a, z_d], dim=1))


class MiSegNetModel(nn.Module):
    def __init__(self, seg_encoder, seg_decoder):
        super().__init__()
    ...
    def forward(self, x):
        ...
        z = self.seg_encoder(normalized)
        return self.seg_decoder(z)
