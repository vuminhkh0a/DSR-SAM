...
def test_mi_segnet_on_target(seg_encoder, seg_decoder, target_loader, device, source_label, target_name='OTU', write_results=True):
    seg_encoder.eval()
    seg_decoder.eval()
    ...
    if write_results:
        save_results(f'mi_segnet_s_{source_label}_t_{target_name}', {
            'dice': round(dice, 2),
            'iou': round(iou, 2),
