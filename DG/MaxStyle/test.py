...
def test_maxstyle_on_target(model, target_loader, device, source_label, target_name, write_results=True):
    model.eval()
    ...
    if write_results:
        save_results(f'maxstyle_s_{source_label}_t_{target_name}', {
            'dice': round(dice, 2),
            'iou': round(iou, 2),
