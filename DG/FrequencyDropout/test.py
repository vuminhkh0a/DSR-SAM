"""
Frequency Dropout Testing

Evaluates trained FD model on held-out target domains.
No FD filters are applied during inference (identity).
"""
import torch
import numpy as np
...
def test_fd_on_target(model, target_loader, device, source_label, target_name, write_results=True):
    """Evaluate the FD model on a target domain."""
    model.eval()
    ...
    if write_results:
        save_results(f'fd_s_{source_label}_t_{target_name}', {
            'dice': round(dice, 2),
            'iou': round(iou, 2),
