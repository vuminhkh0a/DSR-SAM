...
(Medical Image Analysis 2024, arXiv:2309.08842, repo cchen-cc/MA-SAM).

Pure-PyTorch port of the official repo's bbox variant
(MA-SAM/sam_fact_tt_image_encoder_bbox.py + trainer_bbox.py):

  * SAM ViT-H image encoder with lightweight 3D adapters inserted into
    every transformer block (before and after the attention layer).
  * FacT (Factorized Tensor) low-rank adaptation of the query/value
    projections: shared factors U/V (d x r / r x d) and per-block factors
    q_FacTs / v_FacTs (r x r), with s = 1.
  * All SAM pre-trained image-encoder weights are frozen; only the 3D
    adapters, FacT factors, the prompt encoder and the mask decoder are
    updated (fully fine-tuned mask decoder with progressive upsampling).
  * Mask decoder outputs num_classes + 1 masks (channel 0 = background),
    trained with bounding-box prompts.
...
class Block(nn.Module):
    """ViT block with two lightweight 3D adapters (repo image_encoder.py)."""
    ...
    def __init__(self, dim: int, num_heads: int, mlp_ratio: float = 4.0,
        ...
        # 3D adapters (before and after the attention layer)
        ...
        self.adapter_conv = nn.Conv3d(self.adapter_channels, self.adapter_channels,
                                      kernel_size=(3, 1, 1), padding='same')
        ...
        self.adapter_conv_2 = nn.Conv3d(self.adapter_channels, self.adapter_channels,
                                        kernel_size=(3, 1, 1), padding='same')
...
# ============================================================
# Mask decoder with progressive upsampling
# (repo segment_anything/modeling/mask_decoder_bbox.py)
# ============================================================
...
class MaskDecoder(nn.Module):
    def __init__(self, *, transformer_dim: int, transformer: nn.Module,
        ...
        # Progressive upsampling: 4 transposed-conv layers restore the
        # predictions to the full input resolution (paper Sec. 3.4).
...
# ============================================================
# MA-SAM model (repo sam_fact_tt_image_encoder_bbox.py)
# ============================================================

class _Fact_tt_qkv(nn.Module):
    """qkv linear with FacT low-rank increments on q and v."""

    def __init__(self, qkv: nn.Module, q_FacTs: nn.Module, v_FacTs: nn.Module, s: float) -> None:
        super().__init__()
        self.qkv = qkv
    ...
    def forward(self, x: torch.Tensor, FacTu: nn.Module, FacTv: nn.Module) -> torch.Tensor:
        qkv = self.qkv(x)
...
class _Fact_tt_Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, FacTu: nn.Module, FacTv: nn.Module) -> torch.Tensor:
        B, H, W, _ = x.shape
        qkv = self.Attention.qkv(x, FacTu, FacTv).reshape(
            B, H * W, 3, self.Attention.num_heads, -1).permute(2, 0, 3, 1, 4)
        ...
        if self.Attention.use_rel_pos:
            attn = add_decomposed_rel_pos(
                attn, q, self.Attention.rel_pos_h, self.Attention.rel_pos_w, (H, W), (H, W))
        attn = attn.softmax(dim=-1)
        x = (attn @ v).view(B, self.Attention.num_heads, H, W, -1).permute(
...
class _Fact_tt_Block(nn.Module):
    """Transformer block with two 3D adapters and FacT attention."""

    def __init__(self, block: nn.Module) -> None:
        super().__init__()
        self.Block = block

    def _adapter(self, x, FacTu, FacTv, d_size, second=False):
        b_size, hw_size = x.shape[0], x.shape[1]
        if second:
            norm, down, conv, act, up = (self.Block.adapter_norm_2,
                                         self.Block.adapter_linear_down_2,
                                         self.Block.adapter_conv_2,
                                         self.Block.adapter_act_2,
                                         self.Block.adapter_linear_up_2)
        else:
            norm, down, conv, act, up = (self.Block.adapter_norm,
                                         self.Block.adapter_linear_down,
                                         self.Block.adapter_conv,
                                         self.Block.adapter_act,
                                         self.Block.adapter_linear_up)
        shortcut = x
        x = norm(x)
        x = down(x)
        x = x.contiguous().view(int(b_size / d_size), d_size, hw_size, hw_size,
                                self.Block.adapter_channels)
    ...
    def forward(self, x: torch.Tensor, FacTu: nn.Module, FacTv: nn.Module, d_size: int) -> torch.Tensor:
        x = self._adapter(x, FacTu, FacTv, d_size, second=False)
        ...
        x = self._adapter(x, FacTu, FacTv, d_size, second=True)
        x = x + self.Block.mlp(self.Block.norm2(x))
        return x


class _Fact_tt_ImageEncoderViT(nn.Module):
    def __init__(self, image_encoder: nn.Module, FacTu: nn.Module, FacTv: nn.Module) -> None:
        super().__init__()
        self.ImageEncoderViT = image_encoder
        self.FacTu = FacTu
        self.FacTv = FacTv
...
class Sam(nn.Module):
    ...
    def forward(self, batched_input, multimask_output, image_size, bbox_input):
        return self.forward_train(batched_input, multimask_output, image_size, bbox_input)
...
class Fact_tt_Sam(nn.Module):
    """Applies low-rank FacT adaptation to the SAM image encoder (repo)."""

    def __init__(self, sam_model: Sam, r: int = 32, fact_layer=None, s: float = 1.0) -> None:
        super().__init__()
        assert r > 0
        ...
        # Freeze all pre-trained image-encoder weights except the adapters.
        for k, v in sam_model.image_encoder.named_parameters():
            if '.adapter_' not in k:
                v.requires_grad = False

        # Add FacT factors to the query/value projections of each block.
        ...
        sam_model.image_encoder = _Fact_tt_ImageEncoderViT(
            sam_model.image_encoder, self.FacTu, self.FacTv)
        self.sam = sam_model
...
def build_masam(checkpoint=None, model_type='vit_h', image_size=256,
                num_classes=1, rank=32, scale=1.0):
    """Build the MA-SAM model (Fact_tt_Sam wrapper around SAM ViT-H)."""
    assert model_type == 'vit_h'
    sam, _ = _build_sam(image_size=image_size, num_classes=num_classes,
                        checkpoint=checkpoint)
    net = Fact_tt_Sam(sam, r=rank, s=scale)
    return net
